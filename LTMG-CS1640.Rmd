...
```{r}
File_data <- as.matrix(read.csv("BEELINE-data/inputs/Curated/GSD/GSD-2000-1/ExpressionData.csv", sep = ',', header = TRUE, row.names = 1))
File_data <- File_data[rowSums(File_data) > 0, colSums(File_data) > 0]
Zcut_G<-log(Global_Zcut(File_data))
File_data
```
...
```{r}
File_LTMG$State
write.table(t(as.matrix(File_LTMG$State)), 'state_expression_temp.tsv', sep = "\t", col.names = NA, row.names = TRUE)
```
